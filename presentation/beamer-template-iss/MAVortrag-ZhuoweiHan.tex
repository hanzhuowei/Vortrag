...
% 	\end{minipage}
 ...
 \subsection{CRBM}
	...
	\begin{frame}[t]{Training of Energy-based Model}
	 \alert<1-1>{Maximum Likelihood Estimation} $P(\x|\btheta)$\\
	 \begin{columns}
	 \column{2in}
	...
	\begin{frame}[t]{Training of Energy-based Model}
		...
		\begin{align}\label{loggra}
			- \frac{\partial  \log P(\mathbf{x})}{\partial \boldsymbol{\theta}} = \frac{\partial \mathcal{F}(\mathbf{x})}{\partial \boldsymbol{\theta}} -\alert<3->{
			      \sum_{\tilde{\mathbf{x}}} P(\tilde{\mathbf{x}})
				  \frac{\partial \mathcal{F}(\tilde{\mathbf{x}})}{\partial \boldsymbol{\theta}}\nonumber}
		\end{align}
		\only<3->{\column{1.5in}
% 		\[ \xleftarrow{\hspace*{3mm}} \]
		$\leftarrow$intractable!}
		\end{columns}
		}
		\only<4->{
		\begin{columns}
		\column{3in}
		  \begin{align}
			- \frac{\partial  \log P(\mathbf{x})}{\partial \boldsymbol{\theta}} = \frac{\partial \mathcal{F}(\mathbf{x})}{\partial \boldsymbol{\theta}} - \alert<4-4>{ \frac{1}{|\mathcal{N}|}
